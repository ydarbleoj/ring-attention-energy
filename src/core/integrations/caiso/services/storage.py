"""Storage manager for CAISO data with Polars/Parquet operations."""
...
class StorageManager:
    """Manages Polars DataFrame storage operations with Parquet format for CAISO data."""
    ...
    def save_dataframe(
        ...
        if df.is_empty() and len(df.columns) == 0:
            raise ValueError("Cannot save empty DataFrame with no columns")
        ...
        logger.info(f"Saved CAISO DataFrame with {len(df)} rows to {file_path}")
        return file_path
    ...
    def load_dataframe(
        ...
        logger.info(f"Loaded CAISO DataFrame with {len(df)} rows from {file_path}")
        return df
    ...
    def delete_file(self, filename: str, subfolder: Optional[str] = None) -> bool:
        ...
        if file_path.exists():
            file_path.unlink()
            logger.info(f"Deleted CAISO file {file_path}")
            return True
        else:
            logger.warning(f"CAISO file {file_path} not found for deletion")
            return False